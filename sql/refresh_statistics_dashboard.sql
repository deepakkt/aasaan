...
where ps.id=psc.program_id and p.id = ps.program_id and c.zone_id = z.id and ps.center_id = c.id and psc.category_id=9
group by 1, 2,3;
...
insert into statistics_statisticsprogramcounts(
zone_name, program_name, program_window, program_count,participant_count) select ps.zone, 'UyirNokkam' as program_name, to_char(date_trunc('month', ps.date),'yyyy-mm') AS program_window, ps.no_of_classes,
