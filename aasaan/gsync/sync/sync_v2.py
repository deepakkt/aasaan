...
class SheetSync:
    ...
    def _init_core(self, gc, sheet_key):
    ...
    def sync(self, cache=False):
        def _parse_tracker(result):
            _worksheet = -1
            _worksheet_row = 0
            _worksheet_start_row = 2
            _worksheet_columns = 35
...
class ScheduleSync(SheetSyncCache):
    ...
    def get_programschedule_queryset(self):
        ...
        _program_schedule_initial =  ProgramSchedule.objects.filter(start_date__gte=forty_five_days_ago).order_by('center__zone',
                                                                                            '-start_date',
                                                                                            'center', 'program')
        return _program_schedule_initial.exclude(program__name='Satsang')
